...
const fakeAccounts = [
	...
	{ username: 'anotherspectra', alias: 'gold smurf', agent: 'Sova' },
...
export default function AccountList() {
	...
	return <div className="group relative w-full h-full py-6 px-12">
		<div className={`flex gap-4 overflow-x-auto overflow-y-auto p-1 ${fakeAccounts.length > 2 ? 'pb-3' : ''}`}>
			...
			<AccountCard username={'new-account'} alias={'Make account'} agent={'Sage'} />
			{/* </Link> */}
		</div>
		{/* Hint */}
		<div className="relative">
			{fakeAccounts.length > 2 && <div className="absolute right-0 flex pt-4 items-center justify-end transition-all duration-150 ease-in-out text-zinc-800 group-hover:text-zinc-300">
				<p>
