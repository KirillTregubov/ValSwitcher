...
export default function AccountList({ className }) {
	...
	return <div className={`group relative w-full h-full py-6 px-12 ${className}`}>
		<div className={`flex gap-4 overflow-x-auto overflow-y-auto p-1 ${fakeAccounts.length > 2 ? 'pb-3' : ''}`}>
			...
			{/* <Link to="/new-account" state={{ backgroundLocation: location }}> */}
			<AccountCard alias={'Make account'} agent={'Sage'} />
			{/* </Link> */}
		</div>
		{/* Hint */}
		<div className="relative">
			{fakeAccounts.length > 2 && <div className="text-sm absolute right-0 flex pt-3 items-center justify-end transition-all ease-out duration-300 text-zinc-800 group-hover:text-zinc-300 select-none">
				<p className="">Hold <span className="p-1 px-[0.35rem] bg-zinc-900 border-b-2 border-zinc-900 group-hover:bg-zinc-800 group-hover:border-zinc-600 rounded-md">shift</span> to use</p>
				<svg alt="scrollwheel" className="ml-[0.1rem] fill-current" width="24px" height="24px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
